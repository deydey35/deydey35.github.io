...
insert into _academie (code_academie, lib_academie)
  ...
  from _temp_fr_en_etablissements_ep
  where (code_academie, libelle_academie) not in (select code_academie, lib_academie from _academie)and code_academie is not null ;
...
update _academie
set lib_academie = replace(lib_academie,'C','Ç');
...
insert into  _quartier_prioritaire (code_quartier_prioritaire, nom_quartier_prioritaire)
  ...
  from _temp_fr_en_etablissements_ep
  where (qp_a_proximite, nom_du_qp) not in (select code_quartier_prioritaire, nom_quartier_prioritaire from _quartier_prioritaire)and qp_a_proximite is not null ;

--- et tout le reste

commit;

create view VUE_quatier(code,nom) AS
select distinct qp_a_proximite, nom_du_qp
  from _temp_fr_en_etablissements_ep
  where (qp_a_proximite, nom_du_qp) not in (select code_quartier_prioritaire, nom_quartier_prioritaire from _quartier_prioritaire)and qp_a_proximite is not null ;

SELECT distinct * FROM VUE_quatier;
...
SELECT DISTINCT 
    ...
    nombre_eleves_segpa,
   nombre_eleves_ulis
FROM _temp_fr_en_indicateurs_valeur_ajoutee_colleges AS t1
INNER JOIN _temp_fr_en_college_effectif_niveau_sexe_lv AS t2 
ON t1.uai = t2.numero_college
WHERE t2.rentree_scolaire = '2019-2020';

SELECT * FROM view_data;
copy (SELECT * FROM view_data) TO '/home/etuinfo/dnaicker/Documents/SEMESTRE 2/SAE 2.04/Stats/VUE.csv' WITH CSV HEADER
